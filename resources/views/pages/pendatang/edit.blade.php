...
@section('content')
  <div class="card shadow mb-4 col-12">
    <div class="card-header">
      <a href="/pendatang" class="btn btn-warning text-white"><i class="bi bi-box-arrow-left"></i> Kembali</a>
    </div>
    <div class="card-body">
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <select name="nik" class="form-select selectpicker w-100" data-live-search="true">
              <option selected>{{ $pendatang->warga->nik }}</option>
              @foreach ($warga as $w)
                <option value="{{ $w->id }}"{{ $w->id == $pendatang->warga->warga_id ? 'selected' : '' }}>
                  {{ $w->nik }}
                </option>
              @endforeach
            </select>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <select name="pendatang" class="form-select selectpicker w-100" data-live-search="true">
              <option selected>{{ $pendatang->warga->nama }}</option>
              @foreach ($warga as $w)
                <option value="{{ $w->id }}"{{ $w->id == $pendatang->warga->warg_id }}>{{ $w->nama }}</option>
              @endforeach
            </select>
          </div>
        </div>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <select class="form-select" name="jenis_kelamin">
              <option selected>{{ $pendatang->jenis_kelamin }}-</option>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <input type="date" name="tanggal_datang" value="{{ old('tanggal_datang') ?? $pendatang->tanggal_datang }}"
              placeholder="tanggal datang" class="form-control" required>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <select class="form-select selectpicker w-100" name="warga_id" data-live-search="true">
              <option selected>{{ $pendatang->warga->nama }}</option>
              @foreach ($warga as $w)
                <option value="{{ $w->id }}"{{ $w->id == $pendatang->id ? 'selected' : '' }}>{{ $w->nik }}
                  -
                  {{ $w->nama }}</option>
              @endforeach
            </select>
